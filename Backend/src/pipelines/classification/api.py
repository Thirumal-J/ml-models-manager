...
parentDir = os.path.abspath(os.path.join(os.getcwd(), os.pardir)) #  Gives Pipelines folder
...
def get_model_results(algorithmName, X_train_balanced, X_test, y_train_balanced, y_test):
	...
	classifier = ""
	algorithms = appConf.CLASSIFICATION_ALGORITHMS
	if algorithmName == "Random Forest Classification":
		classifier = RandomForestClassifier(n_estimators=100, random_state=0)
	elif algorithmName == "K-Nearest Neighbours Classification":
		classifier = KNeighborsClassifier(n_neighbors = 5, metric = 'minkowski', p = 2)
	elif algorithmName == "Naive Bayes Classification":
		classifier = GaussianNB()
	elif algorithmName == "Decision Tree Classification":
	...
	print(f"--Is directory exists or not --> {os.path.exists(sourceDir + appConf.TEMP_PICKLE_FILE_LOCATION)}")
